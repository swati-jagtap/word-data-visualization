...
let d='';         //no graph present
const cCount = document.querySelector('.cCount');
cCount.textContent = countries.length+ " ";
const button =document.querySelectorAll('button');
//console.log(button)
button.forEach((b)=>b.innerHTML = (b.textContent.toUpperCase()))

const populationB = document.querySelector('.population');
const languageB = document.querySelector('.languages'); 
const graphWrapper = document.querySelector('.graph-wrapper');



populationB.addEventListener('click',function(){
  //graphWrapper.innerHTML = lcA.sort(function(a,b){return b.c-a.c})
  h4.textContent = "10 Most populated countries in the world";
  if(d==2)
  {
    graphWrapper.replaceChild(tableP,tableL);
    d=1;
  }

})

languageB.addEventListener('click',function(){
  h4.textContent = "10 Most spoken languages in the world";
       if(d==1){
        graphWrapper.replaceChild(tableL,tableP); 
        d=2;
       }
})
...
//countries.forEach((ele)=>console.log(ele['name']));

const population = [];
let wp=0;
countries.forEach((ele)=>{
  wp=wp+ele['population'];
})

countries.forEach((ele)=>{
  const p = ele['population'];
 // wp=wp+p;  
  const n = ele['name'];
  //console.log(p)
  const populationName = {p,n};
...
//console.log(population.sort(function(a, b){return b.p-a.p}))
population.sort(function(a, b){return b.p-a.p})
let i=1;
for(let obj of population){
  if(i<=10){
    console.log(obj.n)
    i++;
  }else{
...
for(i=0;i<=10;i++)
{
  ...
  for(let j=0;j<=2;j++)
  {
    ...
    if(j==0)
    {
      td.textContent =population[i]['n'] ;
      td.style.width = '20%';
    }
    if(j==1)
    {
      const w=[]
      const plw = Math.round((population[i]['p']*100)/wp);
      w.push(plw+"%")
      //console.log(w)
      td.style.width = '80%';
      ...
      progress.style.backgroundColor = 'rgb(228, 179, 20)';
...
if(d==''){
  graphWrapper.appendChild(tableP);
  d=1;           //population graph present
}
...
language =[];
lUniqu=[];
countries.forEach((ele)=>language.push(ele['languages']));
...
for(let s=0;s<lUniqu.length;s++){
  c=0;
  for(j=s;j<lUniqu.length;j++){
    if(lUniqu[s]==lUniqu[j]){
      c=c+1;
    }else{
     // console.log(c)
      l=lUniqu[s];
...
//console.log(lcA.sort(function(a,b){return b.c-a.c}));
lcA.sort(function(a,b){return b.c-a.c});
...
//graphWrapper.appendChild(tableL);
